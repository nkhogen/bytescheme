...
public class RemoteObjectServer implements RemoteObjectListener {
	...
	public RemoteObjectServer(SecurityProvider securityProvider, boolean autoRegisterRemoteObject) {
	...
	public MethodCallResponse login(LoginCallRequest request) {
		...
		try {
			...
			Session session = securityProvider
					.authenticate(new Authentication(request.getUser(), request.getPassword()));
			if (session == null) {
				throw new RemoteMethodCallException(String.format("Failed to authenticate user %s", request.getUser()));
			}
	...
	public MethodCallResponse logout(LogoutCallRequest request) {
		...
		try {
			...
			Session session = securityProvider.destroySession(request.getSessionId());
			if (session != null) {
				response.setReturnValue(session.getId());
			}
		} catch (RemoteMethodCallException e) {
	...
	public MethodCallResponse process(MethodCallRequest request) {
		...
		try {
			...
			Preconditions.checkNotNull(method, "Unknown method name %s with parameter count %s", request.getName(),
					parameterCount);
	...
	private void checkSecurity(MethodCallRequest request) {
		...
		try {
			if (securityProvider.authorize(request)) {
				return;
			}
		} catch (Exception e) {
			LOG.error("EError occurred in security check", e);
			throw new RemoteMethodCallException("Error occurred in security check", e);
		}
		throw new RemoteMethodCallException("Unauthorized user");
	}
	...
	private class ClassMetaData {
		...
		public void addMethod(Method method) {
			String methodKey = String.format(METHOD_KEY_FORMAT, method.getName(), method.getParameterCount());
			LOG.info("Adding method {} in class {}", methodKey, method.getDeclaringClass());
			Method currentMethod = methods.put(methodKey, method);
			Preconditions.checkArgument(currentMethod == null, "Method %s with parameter count %d already exists",
